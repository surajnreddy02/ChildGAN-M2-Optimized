...
for batch in range(num_batches):
    batch_image = images[batch*batch_size:(batch+1)*batch_size,:,:,:].repeat(5, 1, 1, 1).to(device)
    ...
    fixed_g_v = Variable(fixed_g).to(device)
    fixed_l_v = Variable(fixed_l).to(device)
