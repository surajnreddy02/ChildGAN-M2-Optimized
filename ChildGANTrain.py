...
# Device setup for M2 Mac (MPS) and other systems
if torch.backends.mps.is_available() and torch.backends.mps.is_built():
    device = torch.device("mps")
    print("🚀 Using device: MPS (Apple M2 GPU)")
...
# Set MPS high watermark ratio for better memory management on M2
if device.type == "mps":
...
parser.add_argument('--dataroot', type=str, default='./MRCD_Indian_Combined',
...
parser.add_argument('--dz', default=None,
                    help="path to decoder (to continue training)")
parser.add_argument('--outf', default='./training_output',
...
parser.add_argument('--img_interval', type=int, default=50, help='number of iterations between each image saving, default=100')
parser.add_argument('--ngpu', type=int, default=1,
                    help='number of GPUs to use')
args = parser.parse_args()

# Update device usage based on our device detection
use_cuda = device.type in ['cuda', 'mps']
n_z = 50
n_l = 5  # Keep current dataset structure (5 age ranges, expanded later)
n_channel = 3
n_disc = 16
n_gen = 64
n_age = int(n_z/n_l) #10
...
if device.type == 'cuda':
    ...
    # MPS uses the same manual seed as CPU
    torch.manual_seed(args.manual_seed)

# Update warning message for device availability
if (torch.cuda.is_available() or torch.backends.mps.is_available()) and not use_cuda:
    print(f"WARNING: You have a {device.type.upper()} device, so you should probably enable GPU usage")
...
if use_cuda:
    ...
    latent_labels = latent_labels.to(device)
...
## fixed variables to regress / progress age - temporary placeholder
fixed_l_v = Variable(torch.ones(1, 5))  # Will be overridden with correct size
if use_cuda:
    fixed_l_v = fixed_l_v.to(device)
encoder.train()
decoder.train()
train_loss = 0
d_loss = []
...
for epoch in range(args.niter):
    ...
    for g_iter in range(5): # generator  5 times
        ...
        for d_iter in range(1): # dis img 1 time
            ...
            img_age = (img_label // 2).long()  # size = no of image * 1 (integer division and convert to long)
            img_gender = img_label % 2 * 2 - 1  # size = no of image * 1
            img_age_v = Variable(img_age).view(-1, 1)
            img_gender_v = Variable(img_gender.float())
            if epoch == 0 and i==0:
               # Use current batch size for consistency
               current_batch_size = img_data.size(0)
               num_samples = min(8, current_batch_size)
               fixed_noise = img_data[:num_samples]  # Keep original size
               fixed_g = img_gender[:num_samples].view(-1, 1)  # Match batch size
               fixed_img_v = Variable(fixed_noise)
               fixed_g_v = Variable(fixed_g)
               
               # Create fixed age labels matching the actual batch size
               fixed_l = -torch.ones(num_samples, 5)  # Match batch size
               for idx in range(num_samples):
                   age_idx = idx % 5  # Cycle through age categories
                   fixed_l[idx, age_idx] = 1
               fixed_l_v = Variable(fixed_l)
               pickle.dump(fixed_noise, open("fixed_noise.p", "wb"))

            if use_cuda and epoch == 0 and i==0:
                    fixed_img_v = fixed_img_v.to(device)
                    fixed_g_v = fixed_g_v.to(device)
                    fixed_l_v = fixed_l_v.to(device)
            ...
            if use_cuda:
                    ...
                    img_age_v = img_age_v.to(device)
                    img_gender_v = img_gender_v.to(device)

            # make one hot encoding version of label
            batchSize = img_data_v.size(0)
            age_ohe = one_hot(img_age, batchSize, n_l, use_cuda, device)  # size = noOfImages * n_l
            ...
            real_label_dim = Variable(torch.ones(batchSize, 64).fill_(1)).view(-1, 1).to(device)
            fake_label_dim = Variable(torch.ones(batchSize,64).fill_(0)).view(-1, 1).to(device)

            if use_cuda:
                z_star, real_label, fake_label = z_star.to(device), real_label.to(device), fake_label.to(device)
            ...
            eta = torch.FloatTensor(args.batch_size, 1, 1, 1).uniform_(0, 1).to(device)
            eta = eta.expand(args.batch_size, img_data.size(1), img_data.size(2), img_data.size(3))

            interpolated = eta * img_data.to(device) + ((1 - eta) * recon.to(device))
